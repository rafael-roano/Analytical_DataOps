...
so = cleaning_session.read.parquet("/usr/local/spark/resources/output/Extracted_MySQL_Tables/daily_transactions/r_so")
rows = so.count()
logger.info(f"Parquet file 'r_so' was successfully loaded into DataFrame. {rows} rows loaded")
soitem = cleaning_session.read.parquet("/usr/local/spark/resources/output/Extracted_MySQL_Tables/daily_transactions/r_soitem")
rows = soitem.count()
...
rows =not_req.count()
logger.info(f"Row count is {rows}")
not_req.write.mode('overwrite').csv("/usr/local/spark/resources/output/daily_transactions/transactions_not_loaded.csv")
