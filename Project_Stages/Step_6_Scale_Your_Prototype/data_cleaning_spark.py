...
so = cleaning_session.read.parquet("/usr/local/spark/resources/output/Extracted_MySQL_Tables/r_so")
rows = so.count()
logger.info(f"Parquet file 'r_so' was successfully loaded into DataFrame. {rows} rows loaded")
soitem = cleaning_session.read.parquet("/usr/local/spark/resources/output/Extracted_MySQL_Tables/r_soitem")
rows = soitem.count()
logger.info(f"Parquet file 'r_soitem' was successfully loaded into DataFrame. {rows} rows loaded")
product = cleaning_session.read.parquet("/usr/local/spark/resources/output/Extracted_MySQL_Tables/r_product")
rows = product.count()
logger.info(f"Parquet file 'r_product' was successfully loaded into DataFrame. {rows} rows loaded")
part = cleaning_session.read.parquet("/usr/local/spark/resources/output/Extracted_MySQL_Tables/m_part")
rows = part.count()
...
rows =not_req.count()
logger.info(f"Row count is {rows}")
not_req.write.mode('overwrite').csv("/usr/local/spark/resources/output/not_required_trans.csv")
...
logger.info(f"'data_cleaning_spark' script was successfully executed. Runnig time was {script_time} secs")